...
interface StructuredData {
  ...
  local_transport?: {
    auto_rickshaw?: {
      average_base_fare: string;
      per_km_rate: string;
      best_for: string;
    };
    uber?: {
      average_fare_range: string;
...
const StructuredResponse: React.FC<{
  ...
  return (
    <div className="flex flex-col gap-8 w-full mt-4">
      ...
      {/* ── 1. Route Summary ── */}
      {data.route_summary &&
        (data.route_summary.origin ||
          data.route_summary.destination ||
          data.route_summary.from ||
          data.route_summary.to) && (
          <motion.div
                ...
                <div className="flex items-center gap-2 flex-1">
                  ...
                  <div className="flex-1 flex flex-col items-center px-2">
                    <div className="flex items-center gap-1 w-full relative">
                      ...
                      {(data.route_summary.best_mode ||
                        data.route_summary.recommended_travel_mode) && (
                    ...
                    <div className="text-[10px] text-teal-600 font-bold mt-1.5">
                      {data.route_summary.distance_km
                        ? `${data.route_summary.distance_km} km`
                        : ""}
                    </div>
                  </div>
                  <div className="text-center">
      ...
      {/* ── 2. Budget Summary ── */}
      {data.budget_summary &&
        (data.budget_summary.budget_trip_estimate ||
          data.budget_summary.total_estimated) && (
          <motion.div
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 rounded-2xl bg-slate-800/50 border border-slate-700/50 flex flex-col gap-2">
                  ...
                  <span className="text-[9px] text-slate-500 leading-tight">
                    Hostels, local food, sleeper trains
                  </span>
                </div>
                <div className="p-4 rounded-2xl bg-indigo-500/10 border border-indigo-500/20 flex flex-col gap-2">
                  ...
                  <div className="text-xl font-black text-white truncate tracking-tight">
                    {data.budget_summary.mid_range_estimate ||
                      "₹ 15,000 - 25,000"}
                  </div>
                  <span className="text-[9px] text-indigo-200/60 leading-tight">
                    3-star hotels, AC trains, mix of cafes
                  </span>
                </div>
                <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex flex-col gap-2">
                  ...
                  <span className="text-[9px] text-amber-200/60 leading-tight">
                    5-star stay, private car, luxury dining
                  </span>
                </div>
              </div>
              ...
              {data.budget_summary.savings_tip && (
                <div className="mt-2 flex items-start gap-2 p-3 rounded-xl bg-indigo-500/5 border border-indigo-500/10">
                  ...
                  <p className="text-[11px] text-slate-400 italic font-medium leading-relaxed">
                    <span className="text-indigo-300 font-bold not-italic">
                      Pro Tip:{" "}
                    </span>
                    {data.budget_summary.savings_tip}
                  </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
              ...
              <div className="space-y-1.5">
                <div className="text-sm font-black text-slate-800">
                  {data.local_transport.auto_rickshaw.average_base_fare} Base
                </div>
                <div className="text-[10px] text-amber-600 font-bold">
                  {data.local_transport.auto_rickshaw.per_km_rate} /km
                </div>
                <div className="text-[10px] text-slate-500 italic mt-1 font-medium leading-tight">
                  {data.local_transport.auto_rickshaw.best_for}
                </div>
              </div>
            </div>
          )}
          {data.local_transport.uber && (
            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200">
              ...
              <div className="space-y-1.5">
                <div className="text-sm font-black text-slate-800">
                  {data.local_transport.uber.average_fare_range}
                </div>
                <div className="text-[10px] text-slate-600 font-bold">
                  Availability: {data.local_transport.uber.availability_level}
                </div>
                <div className="text-[10px] text-slate-500 italic mt-1 font-medium leading-tight">
                  Book: {data.local_transport.uber.best_time_to_book}
                </div>
              </div>
            </div>
          )}
          {data.local_transport.metro && (
            <div className="bg-blue-50 rounded-2xl p-4 border border-blue-100">
              ...
              {data.local_transport.metro.available === "Yes" ? (
                <div className="space-y-1.5">
                  <div className="text-sm font-black text-slate-800">
                    {data.local_transport.metro.fare_range}
                  </div>
                  <div className="text-[10px] text-blue-600 font-bold truncate">
                    Station: {data.local_transport.metro.nearest_station}
                  </div>
                </div>
              ) : (
      ...
      {data.nearby_food &&
        ...
        data.nearby_food.some(
          (f: any) =>
            f.restaurant_name || f.name || f.title || f.place || f.location,
        ) && (
          <motion.div
            initial={{ opacity: 0, y: 8 }}
            animate={{ opacity: 1, y: 0 }}
              ...
              {data.nearby_food
                ...
                .map((rest: any, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="p-3 border border-slate-100 rounded-xl hover:border-orange-200 transition-all bg-orange-50/10"
                    >
                      <div className="flex justify-between items-start mb-1">
                        <div className="font-bold text-sm text-slate-800 leading-tight truncate pr-2">
                          {name}
                        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
            {data.famous_food_items.map((food: any, idx) => {
              const name =
                typeof food === "string"
                  ? food
                  : food.dish_name || "Famous Item";
              const area =
                typeof food === "object"
                  ? food.best_area || "Market Area"
                  : "Local Speciality";
              const isVeg =
                typeof food === "object" ? food.veg_or_nonveg === "Veg" : true;
          ...
          <div className="space-y-2">
            {data.itinerary.map((day, idx) => {
              ...
              const isTitleHighlighted =
                activeHighlightIndex === titleIdx && titleIdx !== -1;
              ...
              return (
                ...
                >
                  ...
                  <div
                    id={
                      titleIdx !== -1
                        ? `line-${(data as any).messageId}-${titleIdx}`
                        : undefined
                    }
                    className={cn(
                      "px-4 py-3 flex items-center gap-3 border-b transition-all duration-500",
                      isTitleHighlighted
                        ? "bg-indigo-700 text-white border-indigo-800 shadow-inner"
                  ...
                  <div className="px-5 py-4 space-y-3">
                    <ul className="space-y-2.5">
                      {day.activities.map((act, i) => {
                        ...
                        const isActHighlighted =
                          activeHighlightIndex === actIdx && actIdx !== -1;
                        ...
                        return (
                          <li
                            ...
                            id={
                              actIdx !== -1
                                ? `line-${(data as any).messageId}-${actIdx}`
                                : undefined
                            }
                            className={cn(
                    ...
                    {day.tip &&
                      (() => {
                        ...
                        const isTipHighlighted =
                          activeHighlightIndex === tipIdx && tipIdx !== -1;

                        return (
                          <div
                            id={
                              tipIdx !== -1
                                ? `line-${(data as any).messageId}-${tipIdx}`
                                : undefined
                            }
                            className={cn(
                          ...
                          >
                            <span
                              className={cn(
                                "font-bold uppercase tracking-wider mr-1",
                                isTipHighlighted
                                  ? "text-indigo-200"
                                  : "text-amber-600",
                              )}
